...
using System.Linq;
using System.Net;
using System.Reflection;//2016-04-25 Dllimport 引用添加
...
namespace GRPlatForm
{
    public partial class ServerForm : Form
    {
        ...
        public static EbmSignature ebsign = new EbmSignature();    //签名数据
        public static int ebsign_index = 0;
        public static string s_KeySN = "";
        public static int sNo = 0;

        public string sKeySN = "";
        public string sPriKEY = "";
        public string sPubKEY = "";
        public static Dictionary<string, int> DictKey = new Dictionary<string, int>();
        public ServerForm()
        {
            try
            {
        ...
        private bool InitEBMSign()
        {   
            #region 签名初始化
            bool reflag = false;
            List<string> listKeySN = new List<string>();
            List<string> listPriKey = new List<string>();
            List<string> listPubKey = new List<string>();
            sKeySN = serverini.ReadValue("PLATFORMINFO", "KEYSN","");
            sPriKEY = serverini.ReadValue("PLATFORMINFO", "PRIKEY","");
            sPubKEY = serverini.ReadValue("PLATFORMINFO", "PUBKEY","");

            SingletonInfo.GetInstance().CurrentCertNo= serverini.ReadValue("PLATFORMINFO", "CurrentCertNo");
            s_KeySN = SingletonInfo.GetInstance().CurrentCertNo;
            listKeySN.AddRange(sKeySN.Split(','));
            listPriKey.AddRange(sPriKEY.Split(','));
            listPubKey.AddRange(sPubKEY.Split(','));
            if (listKeySN.Count > 0 && listKeySN.Count == listPriKey.Count && listKeySN.Count == listPubKey.Count)
            {
                try
                {
                    for (int i = 0; i < listKeySN.Count; i++)
                    {
                        EBCert ebcert = new EBCert();
                        ebcert.Cert_sn = listKeySN[i];  //000000000001
                        ebcert.PriKey = listPriKey[i].ToUpper();   //2FB22AEDD02205AF5B8F4171DC1DCBB5A4643D2BF0034CF7D0283CCAE193EF50
                        ebcert.PubKey = listPubKey[i].ToUpper();  //"E45FB09A87658BF8D92D0C1B391D953C66598AEFF8EEE61A88AD993F7F2395651CCFED6465F5062778CFC396D4D9DFC44C2040B8B0E615916ED63B45465753E6";
                        ebsign_index = ebsign.AddEBCert(ebcert);
                        DictKey.Add(listKeySN[i], ebsign_index);
                        reflag = true;
                    }
                }
                catch
        ...
        private void ServerForm_Load(object sender, EventArgs e)  //页面参数初始化
        {
          ...
          //  CloudProducer = serverini.ReadValue("MQInfo", "CloudProducer");
            ...
            if (!InitEBMSign())
            {
                MessageBox.Show("签名信息初始化失败，请检查参数！");
            }
            #endregion
        }
                        ...
                        if (dtMedia != null && dtMedia.Rows.Count > 0)
                        {
                            for (int idtM = 0; idtM < dtMedia.Rows.Count; idtM++)
                            {
                                ...
                                if (TrLL != "")
                                {
                                    ...
                                    {
                                        DV.Longitude = "118.67902";
                                        DV.Latitude = "33.72084";
                                    }
                                }
                                else
                                {
        ...
        private string HEXtoBin(string instr)
        {
            string returnstr = "";
            return returnstr;
        }
        ...
        private void DealTar()
        {
            ...
            string PlayUrlFlag = "0";    //是不是新协议的播放URL  2018-05-23
            ...
            while (bDeal)
            {
                ...
                try
                {
                    while (lDealTarFiles.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            if (File.Exists(lDealTarFiles[0]))
                            {
                                try
                                {
                                    #region  把tar包上传到融合平台的ftp文件夹    20181119  修改
                                    string path = lDealTarFiles[0];
                                    string[] fullpath = path.Split('\\');
                                    string ftppath = fullpath[fullpath.Length-1];
                                    ...
                                    switch (ebd.EBDType)
                                    {
                                        case "EBM":
                                            ...
                                            if ((EBMVerifyState || RealAudioFlag) && strMsgType == "2")//实时流在播发时的停止
                                            {
                                                ...
                                                {
                                                    Log.Instance.LogWrite("LasttarInfo：" +SingletonInfo.GetInstance().lasttarinfo.paramValue+">>"+SingletonInfo.GetInstance().lasttarinfo.Severity+">>"+SingletonInfo.GetInstance().lasttarinfo.sORG_ID2+">>"+SingletonInfo.GetInstance().lasttarinfo.TsCmdStoreID);

                                                    if (ebd.EBDID == "10245000000000001010101011000000000000034")
                                                    {
                                                        SingletonInfo.GetInstance().resendFlag = true;
                                                        lDealTarFiles.RemoveAt(0);//无论是否成功，都移除
                                                    }
                                                    else
                                                    {
                                                        SetText("停止播发1：" + DateTime.Now.ToString(), Color.Red);
                    ...
                    "values('" + ebd.SRC.EBRID + "','" + ebd.EBDID + "', '" + ebd.EBM.MsgBasicInfo.SenderCode + "', '" + ebd.EBM.MsgBasicInfo.SenderName + "','" + ebd.EBM.EBMID + "', '" + ebd.EBM.MsgBasicInfo.SendTime + "','" + ebd.EBM.MsgBasicInfo.StartTime + "','" + ebd.EBM.MsgBasicInfo.EndTime + "','" + ebd.EBM.MsgContent.MsgTitle + "','" + lab_EBMType + "','" + ebd.EBM.MsgContent.MsgDesc + "','" + ebd.EBM.MsgContent.AreaCode + "','" + ebd.EBM.MsgBasicInfo.Severity + "','" + ebd.EBM.MsgContent.Auxiliary.AuxiliaryDesc + "','" + SingletonInfo.GetInstance().CheckEBMStatusFlag + "') SELECT CAST(scope_identity() AS int)";
                                                        ...
                                                        if (strAuxiliaryType != "61" && !EBMVerifyState && SingletonInfo.GetInstance().CheckEBMStatusFlag == "0")
                                                        {
                                                            ListViewItem listItem = new ListViewItem();
                                                            listItem.Text = (list_PendingTask.Items.Count + 1).ToString();
                                                            listItem.SubItems.Add(lDealTarFiles[0]);
                                                ...
                                                try
                                                {
                                                    ...
                                                    #region  把音频文件上传到ftp服务器  20181119   测试注释  20190504
                                                    //if (AudioFileListTmp.Count > 0)
                                                    //{
                                                    ...
                                                    for (int ai = 0; ai < AudioFileListTmp.Count; ai++)
                                                    {
                                                        ...
                                                        //if ((PlayType == "2"))//(AudioFlag == "2")
                                                        {
                                                           ...
                                                           // sAread = ebd.EBM.MsgContent.AreaCode; //区域
                                                            ...
                                                            string paramValue = "1~" + strurl + "~0~1000~128~0~1~2~04~10000~0004~";  //新修改的20181114
                                                            if ((PlayType == "2"))
                                                            {
                                                              ...
                                                              //  SendMQOrder(1, paramValue, TsCmdStoreID);
                                                                ...
                                                                SendMQOrderNew(1, paramValue, TsCmdStoreID, sORG_ID2);  //新增于20181114
                                                                Log.Instance.LogWrite("给刘工发送MQ时间："+ DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                                                                Log.Instance.LogWrite("LasttarInfo：" + paramValue + ">>" + ebd.EBM.MsgBasicInfo.Severity + ">>" + sORG_ID2 + ">>" + TsCmdStoreID);
                                                                #region   保存消息 20181121

                                                                if (ebd.EBM.MsgBasicInfo.Severity=="4")
                                                                {
                                                                    LasttarInfo pp = new LasttarInfo();
                                                                    pp.paramValue = paramValue;
                                                                    pp.Severity = ebd.EBM.MsgBasicInfo.Severity;
                                                                    pp.sORG_ID2 = sORG_ID2;//TsCmd_ValueID
                                                                    pp.TsCmdStoreID = TsCmdStoreID;//TsCmd_ID
                                                                    pp.EBDID = ebd.EBDID;
                                                                    SingletonInfo.GetInstance().lasttarinfo = pp;
                                                                }
        ...
        private void SrvInfromUP(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            string strSRV_ID = "";
            string strSRV_CODE = "";
            ServerForm.DeleteFolder(SRVISourceFilePath);//删除原有XML发送文件的文件夹下的XML
            string frdStateName = "";
            List<Device> lDev = new List<Device>();
            try
            {
                ...
                if (dtMedia != null && dtMedia.Rows.Count > 0)
                {
                    // if (dtMedia.Rows.Count > 100)
                    {
                        ...
                        //for (int i = 0; i < mod; i++)
                        {
                            ...
                            xmlHeartDoc = rHeart.DeviceInfoResponse(lDev, frdStateName);
                            CreateXML(xmlHeartDoc, SRVISourceFilePath + xmlEBMStateFileName);
                            ServerForm.mainFrm.GenerateSignatureFile(SRVISourceFilePath, frdStateName);
                            ServerForm.tar.CreatTar(SRVISourceFilePath, ServerForm.sSendTarPath, frdStateName);//使用新TAR
                            string sHeartBeatTarName = sSendTarPath + "\\" + "EBDT_" + frdStateName + ".tar";
                            HttpSendFile.UploadFilesByPost(sZJPostUrlAddress, sHeartBeatTarName);
                        }
                    }
        ...
        private void TerraceStateUP(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            ServerForm.DeleteFolder(TerraceSSourceFilePath);//删除原有XML发送文件的文件夹下的XML
            string frdStateName = "";
            try
            {
                Random rdState = new Random();
                ...
                xmlHeartDoc = rHeart.platformstateInfoResponse(frdStateName);
                CreateXML(xmlHeartDoc, TerraceSSourceFilePath + xmlEBMStateFileName);

                ServerForm.mainFrm.GenerateSignatureFile(TerraceSSourceFilePath, frdStateName);

                ServerForm.tar.CreatTar(TerraceSSourceFilePath, ServerForm.sSendTarPath, frdStateName);//使用新TAR
                string sHeartBeatTarName = sSendTarPath + "\\" + "EBDT_" + frdStateName + ".tar";
                HttpSendFile.UploadFilesByPost(sZJPostUrlAddress, sHeartBeatTarName);
            }
            catch
        ...
        private void TerraceInfrom(object source, System.Timers.ElapsedEventArgs e)
        {
            ...
            ServerForm.DeleteFolder(TerradcISourceFilePath);//删除原有XML发送文件的文件夹下的XML
            string frdStateName = "";
            try
            {
                Random rdState = new Random();
                ...
                xmlHeartDoc = rHeart.platformInfoResponse(frdStateName);
                CreateXML(xmlHeartDoc, TerradcISourceFilePath + xmlEBMStateFileName);
                ServerForm.mainFrm.GenerateSignatureFile(TerradcISourceFilePath, frdStateName);
                ServerForm.tar.CreatTar(TerradcISourceFilePath, ServerForm.sSendTarPath, frdStateName);//使用新TAR
                string sHeartBeatTarName = sSendTarPath + "\\" + "EBDT_" + frdStateName + ".tar";
                HttpSendFile.UploadFilesByPost(sZJPostUrlAddress, sHeartBeatTarName);
            }
            catch
        ...
        private void TimerCcplayer(object source, System.Timers.ElapsedEventArgs e)
        {
            if (ccplayerStopTime < DateTime.Now)
            {
                try
                {
                    SetText("停止播发2：" + DateTime.Now.ToString() + "EBM文件日期: " + ccplayerStopTime, Color.Red);
                    ccplay.StopCPPPlayer2();
                    ...
                    using (FileStream fs = new FileStream(sAnalysisFileName, FileMode.Open))
                    {
                        ...
                        if (ebd.EBM.MsgBasicInfo.MsgType == "2")
                        {
                            if (MessageBox.Show("请确定是否要下发关机指令", "应急关机包", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
                                == DialogResult.Yes)
                            {
                                SetText("停止播发3：" + DateTime.Now.ToString(), Color.Red);
                                string strSql = string.Format("update PLAYRECORD set PR_REC_STATUS = '{0}' where PR_SourceID='{1}'", "删除", TsCmdStoreID);
                                strSql += " update EBMInfo set EBMState=1 where SEBDID='" + SEBDIDStatusFlag + "' ";
                                strSql += "delete from InfoVlaue";
                                //string strSql = "update PLAYRECORD set PR_REC_STATUS = '删除'";
                        ...
                        if (EBMInfo.DialogResult == DialogResult.OK)
                        {
                            ...
                            if (AudioFileListTmp.Count > 0)
                            {
                                ...
                                for (int ai = 0; ai < AudioFileListTmp.Count; ai++)
                                {
                                    ...
                                    string sORG_IDNew = GetORG_ID(ebd.EBM.MsgContent.AreaCode);
                                    string paramValue = "1~" + strurl + "~0~1000~128~0~1~1";
                                    if ((PlayType == "2"))
                                    {
                                        SetText("音频文件存库，将在指定时间内播放", Color.Blue);
                                        sqlstr = "insert into TsCmdStore(TsCmd_Type, TsCmd_Mode, TsCmd_UserID, TsCmd_ValueID, TsCmd_Params, TsCmd_Date,TsCmd_ExcuteTime,TsCmd_SaveTime, TsCmd_Status,TsCmd_EndTime,TsCmd_Note)" +
                                           "values('播放视频', '区域', 1, " + sORG_IDNew + ", '" + paramValue + "', " + "'" + sDateTime + "'" + ",'" + sDateTime + "','" + sDateTime + "', 0,'" + sEndDateTime + "'," + "'-1'" + ")";
                                        TsCmdStoreID = mainForm.dba.UpdateDbBySQLRetID(sqlstr).ToString();
                            ...
                            else//文本转语音
                            {
                               ...
                               // string sORG_ID = m_AreaCode;//((int)mainForm.dba.getQueryResultBySQL(sqlstr)).ToString();
                                string sORG_IDNew = GetORG_ID(ebd.EBM.MsgContent.AreaCode);
                                sqlstr = "insert into TsCmdStore(TsCmd_Type, TsCmd_Mode, TsCmd_UserID, TsCmd_ValueID, TsCmd_Params, TsCmd_Date, TsCmd_Status,TsCmd_EndTime,TsCmd_Note)" +
                                        "values('音源播放', '区域', 1, " + sORG_IDNew + ", '" + strPID + "', '" + sDateTime + "', 0,'" + sEndDateTime + "'," + "'-1'" + ")";
        ...
        private void EBMStateRequest()
        {
            ...
            try
            {
             ...
             //   rState.SourceAreaCode = ServerForm.strSourceAreaCode;
                ...
                if (ebd == null)
                    return;
                string EBMID=string.Empty;
                if (ebd.EBM != null && ebd.EBM.EBMID != null)
                    EBMID = ebd.EBMStateRequest.EBM.EBMID;
                else
                    return;
                try
                {
                    ...
                    using (FileStream fs = new FileStream(sAnalysisFileName, FileMode.Open))
                    {
                        ...
                        if (ebd.EBM.MsgBasicInfo.MsgType == "2")
                        {
                            if (MessageBox.Show("请确定是否要下发关机指令", "应急关机包", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
                                == DialogResult.Yes)
                            {
                                SetText("停止播发4：" + DateTime.Now.ToString(), Color.Red);
                                string strSql = string.Format("update PLAYRECORD set PR_REC_STATUS = '{0}' where PR_SourceID='{1}'", "删除", TsCmdStoreID);
                                strSql += " update EBMInfo set EBMState=1 where SEBDID='" + SEBDIDStatusFlag + "' ";
                                strSql += "delete from InfoVlaue";
                                //string strSql = "update PLAYRECORD set PR_REC_STATUS = '删除'";
                        ...
                        if (AudioFileListTmp.Count > 0)
                        {
                            ...
                            for (int ai = 0; ai < AudioFileListTmp.Count; ai++)
                            {
                                ...
                                //if ((PlayType == "2"))//(AudioFlag == "2")
                                {
                                    ...
                                    //#region  测试 区域写死  20190219
                                    //ebd.EBM.MsgContent.AreaCode = "451423100000";
                                    //#endregion
                                    string sORG_ID2 = GetORG_ID(ebd.EBM.MsgContent.AreaCode);
                                    //  string paramValue = "1~" + strurl + "~0~1000~128~0~1~1";
        ...
        /// <summary>
        /// 发送国标MQ关机命令  2018-05-22
        /// </summary>
        /// <param name="Type"></param>
        /// <param name="ParamValue"></param>
        /// <param name="TsCmd_ID"></param>
        /// <returns></returns>
        private bool SendMQCloseGB(string ParamValue, string ResourceCode)
        {
            if (!MQStartFlag)
            {
                Console.WriteLine("MQ标识未启用,取消发送!");
            ...
            m_lstProperty = CloseGBMQ(ParamValue, ResourceCode);    //2018-05-22   改成适应国标
            return m_mq.SendMQMessage(true, "Send", m_lstProperty);
        }
        ...
        /// <summary>
        /// 组装MQ-GB关指令      2018-05-22
        /// </summary>
        /// <param name="Type"></param>
        /// <param name="value"></param>
        private List<Property> CloseGBMQ(string value, string ResourceCode)
        {
            //USER_PRIORITY:0       
            //TsCmd_Mode:区域       
            //USER_ORG_CODE:P37       
            ...
            //TsCmd_Params:1       
            //TsCmd_Status:0       
            //TsCmd_Date:2018-05-18 10:08:25       
            //TsCmd_ValueID:63415231000000302010401,63415231000000314010401       
            //TsCmd_Type:终端开关GB
            List<Property> InstallList = new List<Property>();
            Property item = new Property();
            item.name = "TsCmd_Mode";
        ...
        private void FindUserInfo(string Name)
        {
            ...
            if (dtUser.Rows.Count > 0)
            {
                ...
                MQUserInfo.TsCmd_ValueID = dtUser.Rows[0]["ORG_ID"].ToString();  // 先注释  20181114
            }
        }
        ...
        private void timer_verifyCert_Tick(object sender, EventArgs e)
        {
            try
            {
                if (SingletonInfo.GetInstance().VerifyCertFaultStr!="")
                {
                    SetText(SingletonInfo.GetInstance().VerifyCertFaultStr,Color.Red);
                    SingletonInfo.GetInstance().VerifyCertFaultStr = "";
                }
            }
            catch (Exception)
            {
        ...
        /// <returns></returns>
        private int CheckMP3Period(string fullfilename)
        {
           ...
           string tt1 = Regex.Match(dir.GetDetailsOf(item, -1), "\\d:\\d{2}:\\d{2}").Value;//
            string[] times = tt1.Split(':');
            int Seconds = 0;

            if (times[0] != "00")
            {
                Seconds += Convert.ToInt32(times[0]) * 3600;
            }
            if (times[1] != "00")
            {
                Seconds += Convert.ToInt32(times[1]) * 60;
            }
            if (times[2] != "00")
            {
        ...
        private void timer_resend_Tick(object sender, EventArgs e)
        {
            if (SingletonInfo.GetInstance().resendFlag)
            {
              if(SendMQOrderNew(1, "1~D:\\Abutment\\93\\AudioFiles\\EBDR_8174ED4A54FC8228E3BE7E14695665D9.mp3~0~1000~128~0~1~2~04~10000~0004", "2546", "175", false))
                {
                    SetText("续播成功：" + DateTime.Now.ToString(), Color.Blue);
                }
                else
                {
                    SetText("续播失败：" + DateTime.Now.ToString(), Color.Red);
                }
                SingletonInfo.GetInstance().resendFlag = false;
            }
        }

        private void button5_Click_1(object sender, EventArgs e)
        {
            SendMQOrderNew(1, "1~D:\\Abutment\\93\\AudioFiles\\EBDR_8174ED4A54FC8228E3BE7E14695665D9.mp3~0~1000~128~0~1~2~04~10000~0004", "2546", "175", false);
        }
    }
}
