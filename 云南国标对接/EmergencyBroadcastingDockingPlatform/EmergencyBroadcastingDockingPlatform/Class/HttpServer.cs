...
namespace EmergencyBroadcastingDockingPlatform
{
    ...
    public class HttpProcessor
    {
        ...
        //            #endregion
        //            //判断是数据开头则退出循环
        //            if (dataArray[j][0] == 69 && dataArray[j][1] == 66 && dataArray[j][2] == 68 && dataArray[j][3] == 66)
        //            {
        //                startIndex = j;
        //                break;
        //            }
        ...
        private void PostRequestDeal(StreamReader sr, string sFileForldPath)
        {
            ...
            if (this.httpHeaders.ContainsKey("Content-Length"))
            {
                ...
                try
                {
                    ...
                    //处理接收的文件
                    bool verifySuccess = false;
                    bool isbeatheart = false;
                    DealTarBack(sFilePath, out verifySuccess,ref  isbeatheart);
        ...
        public void DealTarBack(string filepath, out bool PlatformVerifySignatureresule,ref  bool Isbeatheart)
        {
                        ...
                        using (FileStream SignFs = new FileStream(sSignFileName, FileMode.Open))
                        {
                            ...
                            try
                            {
                                ...
                                if (SingletonInfo.GetInstance().SignatureType == "1")
                                {
                                    ...
                                    byte[] pucsingVi = Convert.FromBase64String(sign.SignatureValue);

                                    //0是签名通过
                                    var result = ServerForm.mainFrm.PlatformVerifySignature(xmlArray, ref SingletonInfo.GetInstance().random, pucsingVi);
