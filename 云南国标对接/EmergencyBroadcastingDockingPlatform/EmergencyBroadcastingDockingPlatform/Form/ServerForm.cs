using Apache.NMS;
using EBSignature;
using EmergencyBroadcastingDockingPlatform.AudioMessage.MQAudio;
       ...
       // public static EBD ebd;
        ...
        private void ServerForm_Load(object sender, EventArgs e)  //页面参数初始化
        {
         ...
         //   TextFirst = SingletonInfo.GetInstance().serverini.ReadValue("INFOSET", "TextFirst");//********文转语是否处于优先级1：文转语优先 2：语音优先
            ...
            if (SingletonInfo.GetInstance().SignatureType=="1")
            {
                //软签
                SingletonInfo.GetInstance().IsUseAddCert = SingletonInfo.GetInstance().serverini.ReadValue("AddCertInfo", "IsUseAddCert") == "1" ? true : false;//“1”表示使用增加的证书 2表示不使用增加证书信息
                SingletonInfo.GetInstance().Cert_SN = SingletonInfo.GetInstance().serverini.ReadValue("AddCertInfo", "Cert_SN");
                SingletonInfo.GetInstance().PriKey = SingletonInfo.GetInstance().serverini.ReadValue("AddCertInfo", "PriKey");
                SingletonInfo.GetInstance().PubKey = SingletonInfo.GetInstance().serverini.ReadValue("AddCertInfo", "PubKey");
                ...
                tmp1.Cert_sn = "000000000006";
                tmp1.PriKey = "2FB22AEDD02205AF5B8F4171DC1DCBB5A4643D2BF0034CF7D0283CCAE193EF50";
                tmp1.PubKey = "E45FB09A87658BF8D92D0C1B391D953C66598AEFF8EEE61A88AD993F7F2395651CCFED6465F5062778CFC396D4D9DFC44C2040B8B0E615916ED63B45465753E6";
                int sn = SingletonInfo.GetInstance().InlayCA.AddEBCert(tmp1);
            }
        ...
        {
            string MediaSql = "select a.SRV_PHYSICAL_CODE,a.SRV_LOGICAL_CODE_GB,a.SRV_RMT_STATUS,b.WorkStatus from SRV a inner join srvStatus_national b on a.SRV_PHYSICAL_CODE = b.srv_physical_code";
            ...
            for (int i = 0; i < dtMediaSRV.Rows.Count; i++)
            {
                ...
                tmpone.powersupplystatus = dtMediaSRV.Rows[i]["WorkStatus"].ToString();
                    ...
                    while (lDealTarFiles.Count > 0)
                    {
                       // SetText("解压文件：" + lDealTarFiles[0].ToString(), Color.Green);
                        ...
                        "values('" + ebd.SRC.EBRID + "','" + ebd.EBDID + "', '" + ebd.EBM.MsgBasicInfo.SenderCode + "', '" + ebd.EBM.MsgBasicInfo.SenderName + "','" + ebd.EBM.EBMID + "', '" + ebd.EBM.MsgBasicInfo.SendTime + "','" + ebd.EBM.MsgBasicInfo.StartTime + "','" + ebd.EBM.MsgBasicInfo.EndTime + "','" + ebd.EBM.MsgContent.MsgTitle + "','" + lab_EBMType + "','" + ebd.EBM.MsgContent.MsgDesc + "','" + ebd.EBM.MsgContent.AreaCode + "','" + ebd.EBM.MsgBasicInfo.Severity + "','" + ebd.EBM.MsgContent.Auxiliary.AuxiliaryDesc + "','" + SingletonInfo.GetInstance().CheckEBMStatusFlag + "') SELECT CAST(scope_identity() AS int)";
                                                ...
                                                SingletonInfo.GetInstance().GlobalEBMList.Add(ebd.EBM);
                                            }
                                            else if (strMsgType == "2")
                                            {
                                                ...
                                                EBM deleteone = SingletonInfo.GetInstance().GlobalEBMList.Find(b => b.EBMID.Equals(ebd.EBM.EBMID));
                                                if (deleteone!=null)
                                                {
                                                    SingletonInfo.GetInstance().GlobalEBMList.Remove(deleteone);
                                                }
                                            }
                                            #endregion End
                                            break;
                                        ...
                                        case "EBMStateRequest":
                                            ...
                                            {
                                                lDealTarFiles.RemoveAt(0);//无论是否成功，都移除  先注释 20180820
                                                string ebmidfind = ebd.EBMStateRequest.EBM.EBMID;
                                                EBM selectone= SingletonInfo.GetInstance().GlobalEBMList.Find(b => b.EBMID.Equals(ebmidfind));
                                                ebd.EBM = selectone;
                                                EBMStateRequest(ebd);
            ...
            switch (strOMDType)
            {
                case "EBRDTInfo":
                    ...
                    for (int idtM = 0; idtM < dtMediaSRV.Rows.Count; idtM++)
                    {
                        ...
                        string Longitudetmp = strSRV_GOOGLE.Split(',')[1];
                        string Latitudetmp = strSRV_GOOGLE.Split(',')[0];
                ...
                case "EBMBrdLog"://播发记录
                    ...
                    if (dtMedia != null && dtMedia.Rows.Count > 0)
                    {
                        for (int idtM = 0; idtM < dtMedia.Rows.Count; idtM++)
                        {
                            ...
                            ebmtmp.MsgBasicInfo.SendTime = dtMedia.Rows[idtM]["SendTime"].ToString().Replace('/','-');
                            ...
                            ebmtmp.MsgBasicInfo.StartTime = dtMedia.Rows[idtM]["StartTime"].ToString().Replace('/', '-');
                            ebmtmp.MsgBasicInfo.EndTime = dtMedia.Rows[idtM]["EndTime"].ToString().Replace('/', '-'); ;
        ...
        #region 应急广播平台消息取消播发状态上报函数
        private void CancelPlayReported(EBD ebd, string BrdStateCode)
        {
            try
            {
                XmlDocument xmlHeartDoc = new XmlDocument();
                ...
                ServerForm.DeleteFolder(ServerForm.sEBMStateResponsePath);//删除原有XML发送文件的文件夹下的XML


                string frdStateName = "10" + SingletonInfo.GetInstance().CurrentResourcecode + BBSHelper.GetSequenceCodes();
                string xmlEBMStateFileName = "\\EBDB_" + frdStateName + ".xml";
                xmlHeartDoc = rHeart.EBMStateRequestResponse(ebd, frdStateName, BrdStateCode);
                CreateXML(xmlHeartDoc, ServerForm.sEBMStateResponsePath + xmlEBMStateFileName);
        ...
        {
            PlatformEBRDTStateReported("Full");
        }
        ...
        {
            PlatformEBRDTInfoReported("Full");
        }
        ...
        private List<Device> GetEBRDTStateFromDataBase(string datatype)
        {
            try
            {
                ...
                if (datatype == "Incremental")
                {
                    ...
                    if (dtMedia.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow item in dtMedia.Rows)
                        {
                            ...
                            pp.powersupplystatus = item["WorkStatus"].ToString();
                ...
                else
                {
                    ...
                    string MediaSql2 = "select srv_physical_code,WorkStatus from srvStatus_national";
                    ...
                    foreach (DataRow item1 in dtMedia1.Rows)
                    {
                        ...
                        {
                            //带回传功能的终端
                            DataRow[] drsingle = dtMedia2.Select(string.Format("srv_physical_code='{0}'", item1["SRV_PHYSICAL_CODE"].ToString()));
                            if (drsingle.Length > 0)
                            {
                                foreach (DataRow item2 in dtMedia2.Rows)
                                {
                                    if (item2["srv_physical_code"].ToString() == item1["SRV_PHYSICAL_CODE"].ToString())
                                    {
                                        ...
                                        {
                                            string statustmp = item2["WorkStatus"].ToString();
                                            if (statustmp.Contains("广播"))
                                            {
                                                pp.StateCode = "5";
                                            }
