    ...
    public partial class mainForm : Form
    {
        ...
        private void mainForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                SingletonInfo.GetInstance().SignatureType= SingletonInfo.GetInstance().serverini.ReadValue("USBPSW", "SignatureType");
                //打开密码器
            ...
            using (FileStream SignFs = new FileStream(strPath + sSignFileName, FileMode.Open))
            {
                ...
                try
                {
                    ...
                    if (SingletonInfo.GetInstance().SignatureType=="1")
                    {
                        ...
                        string strpucSignCerSn = "0000" + SingletonInfo.GetInstance().Cert_SN;
                        string nReturn = Platform_CalculateSingature_String(pucSignature, ref SingletonInfo.GetInstance().random);
                        //生成签名文件
                        string xmlSIGNFileName = "\\EBDS_EBDB_" + strEBDID + ".xml";
                        xmlSignDoc = signrp.SignResponse(strEBDID, strpucSignCerSn, nReturn);
        ...
        public int PlatformVerifySignature(byte[] xmlData, ref int random, byte[] strSingBody)
        {
            int Result = -1;
            strSingBody[9] = 0x06;//这是省平台证书在章工的库里面的证书编号
            List<byte> rr = new List<byte>();
            for (int i = 0; i < 70; i++)
            {
                rr.Add(strSingBody[i + 4]);
            }

            if (SingletonInfo.GetInstance().InlayCA.EbMsgSignVerifyWithoutUTC(xmlData, xmlData.Length, rr.ToArray()))
            {
                Result = 0;
            }
            return Result;
        ...
        public string Platform_CalculateSingature_String(byte[] xmlData, ref int random)
        {
            byte[] signature = new byte[64];
            //  SingletonInfo.GetInstance().InlayCA.EbMsgSign(xmlData, xmlData.Length, ref random, ref signature, SingletonInfo.GetInstance().Cert_Index);
            SingletonInfo.GetInstance().InlayCA.EbMsgSignWithoutUTC(xmlData, xmlData.Length, ref signature, SingletonInfo.GetInstance().Cert_Index);

            List<byte> pp = new List<byte>();
            TimeSpan ts = DateTime.UtcNow - new DateTime(1970, 1, 1, 0, 0, 0, 0);
            int ra = (Int32)Convert.ToInt64(ts.TotalSeconds);
            byte[] utc = BBSHelper.Int32toByte(ra);
            pp.AddRange(utc);
            ...
            string str = Convert.ToBase64String(pp.ToArray());
