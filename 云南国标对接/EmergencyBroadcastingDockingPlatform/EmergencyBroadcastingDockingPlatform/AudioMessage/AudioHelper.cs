...
using EmergencyBroadcastingDockingPlatform.AudioMessage.SendMQ;
using System.Windows.Forms;
using System.Threading.Tasks;

namespace EmergencyBroadcastingDockingPlatform.AudioMessage
{
        ...
        public virtual AudioModel PlayReady(int type, string MQIns,string EBMInfoID)
        {
            try
            {
                ...
                else
                {
                    ...
                    #region  测试其他区域
                    //  AreaString = "360781000000";//瑞金市


                    // D:\Program Files(x86)\云应急服务与管理平台\apache - tomcat\webapps\ginkgo\ftp\EBDR_20190426094006.mp3
                    string[] op = AudioModel.PlayingContent.Split('\\');
                    string filename = op[op.Length-1];
                    if (filename== "EBDR_20190426094006.mp3")
                    {
                        AreaString = "532625102101";//八寨村村委
                    }
                    else if (filename == "EBDR_20190426094128.mp3")
                    {
                        AreaString = "532625102202";//芦差塘村委会
                    }
                    else if (filename == "EBDR_20190324162137.mp3")
                    {
                        AreaString = "532625102203";//母子冲村委会
                    }
                    #endregion
