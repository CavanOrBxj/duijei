       ...
       // public static EBD ebd;
        ...
        private void ServerForm_Load(object sender, EventArgs e)  //页面参数初始化
        {
         ...
         //   TextFirst = SingletonInfo.GetInstance().serverini.ReadValue("INFOSET", "TextFirst");//********文转语是否处于优先级1：文转语优先 2：语音优先
            ...
            tSrvState.Elapsed += new System.Timers.ElapsedEventHandler(SrvStateUP);//终端增量状态上报
            tSrvState.AutoReset = true;

            tTerraceState.Elapsed += new System.Timers.ElapsedEventHandler(TerraceStateUP);//平台增量状态上报
            tTerraceState.AutoReset = true;

            tSrvInfo.Elapsed += new System.Timers.ElapsedEventHandler(SrvInfromUP);//终端增量信息上报
            tSrvInfo.AutoReset = true;

            tTerraceInfrom.Elapsed += new System.Timers.ElapsedEventHandler(TerraceInfrom);//平台增量信息上报
                                ...
                                using (FileStream fs = new FileStream(sAnalysisFileName, FileMode.Open))
                                {
                                    ...
                                    switch (ebd.EBDType)
                                    {
                                        case "EBM":
                                            ...
                                            if (strMsgType == "1"|| strMsgType == "3"|| strMsgType == "4"|| strMsgType == "5")
                                            {
                                                //播放
        ...
        {
            PlatformstateInfoReported("Full");
        }
